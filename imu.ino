...
geometry_msgs::Quaternion f;
ros::Publisher chatter("chatter", &Float64_msg);
ros::Publisher imu123("imu", &imu_msg_);
...
// Currently the raw values are in degrees per seconds, deg/s, so we need to multiply by sendonds (s) to
...
f = createQuaternionFromRPY(roll, pitch, yaw);
Serial.print(f.w);
Serial.print(f.x);
Serial.print(f.y);
Serial.print(f.z);
...
imu_msg_.angular_velocity.x = GyroX;
imu_msg_.angular_velocity.y = GyroY;
imu_msg_.angular_velocity.z = GyroZ;
...
imu_msg_.linear_acceleration.x = AccX;
imu_msg_.linear_acceleration.y = AccY;
imu_msg_.linear_acceleration.z = AccZ;
...
imu_msg_.orientation.w = f.w;
imu_msg_.orientation.x = f.x;
imu_msg_.orientation.y = f.y;
imu_msg_.orientation.z = f.z;
imu_msg_.orientation_covariance[0] = 0.0025;
imu_msg_.orientation_covariance[4] = 0.0025;
imu_msg_.orientation_covariance[8] = 0.0025;
imu123.publish(&imu_msg_);

}
void calculate_IMU_error() {
// We can call this funtion in the setup section to calculate the accelerometer and gyro data error. From
here we will get the error values used in the above equations printed on the Serial Monitor.
// Note that we should place the IMU flat in order to get the proper values, so that we then can the
correct values
